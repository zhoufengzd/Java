...
public class FrameRegexDemo extends JFrame
{
	...
	void btnReplace_actionPerformed(ActionEvent e)
	{
		try
		{
			textReplaceResults.setText(textSubject.getText().replaceAll(textRegex.getText(), textReplace.getText()));
	...
	void printMatch()
	{
		try
		{
			textResults.setText("Index of the first character in the match: " + Integer.toString(regexMatcher.start()) + "\n");
			...
			if (regexMatcher.groupCount() > 0)
			{
				...
				for (int i = 1; i <= regexMatcher.groupCount(); i++)
				{
					...
					{
						textResults.append(groupLabel + " start: " + Integer.toString(regexMatcher.start(i)) + "\n");
