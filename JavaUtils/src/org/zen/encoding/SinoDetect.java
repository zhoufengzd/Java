...
class SinoDetect extends Encoding {
	...
	int euc_tw_probability(byte[] rawtext) {
		...
		for (i = 0; i < rawtextlen - 1; i++) {
			if (rawtext[i] >= 0) { // in ASCII range
				...
				dbchars++;
				if (i + 3 < rawtextlen && (byte) 0x8E == rawtext[i] && (byte) 0xA1 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0xB0 && (byte) 0xA1 <= rawtext[i + 2] && rawtext[i + 2] <= (byte) 0xFE && (byte) 0xA1 <= rawtext[i + 3]
	...
	int euc_kr_probability(byte[] rawtext) {
		...
		for (i = 0; i < rawtextlen - 1; i++) {
			...
			if (rawtext[i] >= 0) {
				...
				dbchars++;
				if ((byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xFE && (byte) 0xA1 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0xFE) {
					krchars++;
					totalfreq += 500;
					row = rawtext[i] + 256 - 0xA1;
					column = rawtext[i + 1] + 256 - 0xA1;
		...
		for (i = 0; i < rawtextlen - 1; i++) {
			...
			if (rawtext[i] >= 0) {
				...
				if ((byte) 0x81 <= rawtext[i] && rawtext[i] <= (byte) 0xFE
						&& ((byte) 0x41 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0x5A || (byte) 0x61 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0x7A || (byte) 0x81 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0xFE)) {
					krchars++;
					totalfreq += 500;
					if ((byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xFE && (byte) 0xA1 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0xFE) {
						row = rawtext[i] + 256 - 0xA1;
	...
	int iso_2022_kr_probability(byte[] rawtext) {
		...
		for (i = 0; i < rawtext.length; i++) {
			if (i + 3 < rawtext.length && rawtext[i] == 0x1b && (char) rawtext[i + 1] == '$' && (char) rawtext[i + 2] == ')' && (char) rawtext[i + 3] == 'C') {
				return 100;
			}
		}
		return 0;
	...
	int sjis_probability(byte[] rawtext) {
		...
		for (i = 0; i < rawtextlen - 1; i++) {
			...
			if (rawtext[i] >= 0) {
				...
				dbchars++;
				if (i + 1 < rawtext.length && (((byte) 0x81 <= rawtext[i] && rawtext[i] <= (byte) 0x9F) || ((byte) 0xE0 <= rawtext[i] && rawtext[i] <= (byte) 0xEF))
